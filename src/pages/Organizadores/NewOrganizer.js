...
export default function NewOrganizer() {
    ...
    const Item = ({ name, user, email, telefone }) => (
        <TouchableWithoutFeedback onPress={() => Alert.alert("Organizador", "Tem certeza que deseja tornar " + name + " um organizador?", [{ text: "Não" }, { text: "Sim", onPress: () => updateDoc(doc(firestore, "membros", email), { type: "Organizador" }).then(() => { Alert.alert("Organizadores", name + " agora é um organizador!") }) }])}>
